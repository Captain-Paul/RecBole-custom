...
class SASRecD(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.RATING_SEQ = self.RATING + config['LIST_SUFFIX']
        ...
        self.ra_fuse = nn.Linear(self.hidden_size * 2, self.hidden_size)
    ...
    def forward(self, item_seq, item_seq_len, rating_seq):
        ...
        trm_output, attention_scores = self.trm_encoder(input_emb,feature_emb,position_embedding, extended_attention_mask, output_all_encoded_layers=True)
        output = trm_output[-1]
        seq_last = self.gather_indexes(output, item_seq_len - 1)
        
        rating_seq = rating_seq.to(dtype=output.dtype, device=output.device)   # ★ 关键：对齐 dtype/device
        attention_scores = attention_scores.to(dtype=output.dtype, device=output.device)
        gamma_last = attention_scores[:, :, -1, :]
        beta = torch.softmax(rating_seq.unsqueeze(1) * gamma_last, dim=-1)
        p_g = torch.matmul(beta, output).mean(dim=1)
        seq_output = nn.functional.gelu(self.ra_fuse(torch.cat([seq_last, p_g], dim=-1)))
        return seq_output
    ...
    def calculate_loss(self, interaction):
        ...
        seq_output = self.forward(item_seq, item_seq_len, rating_seq)
